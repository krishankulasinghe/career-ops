// Port of liveness-core.mjs — verbatim logic, zero external dependencies
...
function hasApplyControl(controls: string[] = []): boolean {
  return controls.some((control) => APPLY_PATTERNS.some((p) => p.test(control)));
}
